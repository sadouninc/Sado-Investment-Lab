...
def _position_notionals(
    ...
    for index, item in enumerate(positions):
        ...
        if code not in market_prices:
            ...
            valued.append({"security_code": code, "notional": None})
            continue
        price = _finite_positive(market_prices[code], field=f"market_prices[{code}]", allow_zero=False)
        valued.append({"security_code": code, "notional": quantity * price})
...
def calculate_membership_exposure(
    ...
    *,
    ...
    theme_before, sector_before, unknown_membership = _aggregate_known(valued_positions, memberships)
    ...
    quantity = _finite_positive(action_copy.get("quantity"), field="proposed_action.quantity", allow_zero=False)
    price = _finite_positive(action_copy.get("price"), field="proposed_action.price", allow_zero=False)
    ...
    equity = None if portfolio_equity is None else _finite_positive(
        portfolio_equity, field="portfolio_equity", allow_zero=False
    )
    ...
    for theme in sorted(all_themes):
        delta = sign * notional if target_membership is not None and theme in target_membership["themes"] else 0.0
        theme_results[theme] = _category_projection(
            before=theme_before.get(theme, 0.0), delta=delta, equity=equity, complete=complete
        )

    for sector in sorted(all_sectors):
        delta = sign * notional if target_membership is not None and sector == target_membership.get("sector") else 0.0
        sector_results[sector] = _category_projection(
            before=sector_before.get(sector, 0.0), delta=delta, equity=equity, complete=complete
        )

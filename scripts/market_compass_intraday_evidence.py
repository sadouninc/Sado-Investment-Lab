    ...
    try:
        ...
        adapted = {
            ...
            "sector": {
                "id": sector["id"],
                "label": sector["label"],
                "medium_term_regime": sector["medium_term_regime"],
            },
            "data_quality": {
                "freshness": freshness,
        ...
        raise ValueError(f"Payload validation failed: missing required key {exc}") from exc
    except (TypeError, AttributeError) as exc:
        raise ValueError(f"Payload validation failed: invalid nested structure ({exc})") from exc

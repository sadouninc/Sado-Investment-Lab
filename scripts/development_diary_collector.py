...
IMPLEMENTATION_STATES = {"EXECUTION_EVIDENCE", "IMPLEMENTING", "REVISION_REQUIRED", "CONFLICT_RESOLUTION", "PR_OPEN", "CI_WAIT", "REVIEW_WAIT", "MERGE_READY", "MERGED"}
...
def validate_snapshot(snapshot: dict[str, Any]) -> None:
    jsonschema.Draft202012Validator(load_schema(), format_checker=jsonschema.FormatChecker()).validate(snapshot)
...
def stable_event_id(event: dict[str, Any]) -> str:
    ...
    occurred = event.get("occurred_at") or event.get("created_at") or event.get("merged_at") or event.get("closed_at")
...
def _blank_executor_stats() -> dict[str, Any]:
    return {
        "dispatch_count": 0, "ack_count": 0, "execution_evidence_count": 0,
        "pr_count": 0, "merge_count": 0, "success_count": 0, "failure_count": 0,
        "terminal_noop_count": 0, "elapsed_minutes": None, "lead_time_minutes": None,
        "rework_count": 0, "duplicate_conflict_waste_count": 0,
    }
...
class DailyCollector:
    def __init__(self, diary_date: date):
        ...
        self.exec_stats: dict[tuple[str, str], dict[str, Any]] = defaultdict(_blank_executor_stats)
        self.ready_count = self.implementation_start_count = 0
        self.pr_opened = self.pr_merged = self.pr_closed_unmerged = 0
        self.active_wip = self.waiting_count = self.ready_nonconflicting_count = 0
    ...
    def record(self, event: dict[str, Any]) -> None:
        ...
        if kind == "READY":
            ...
            stats["ack_count"] += 1
        elif kind in IMPLEMENTATION_STATES:
    ...
    def add_correction(self, late_event: dict[str, Any], reason: str, recorded_at: datetime) -> None:
        ...
        if correction_id in self._correction_ids:
            return
        refs = late_event.get("evidence_refs") or [str(late_event.get("ref") or event_id)]
        self.corrections.append({"correction_id": correction_id, "reason": reason, "evidence_refs": list(refs), "recorded_at": recorded_at.isoformat()})
...
def persist_snapshot(snapshot: dict[str, Any], target: Path) -> None:
    """Validate first, then atomically replace target; invalid input cannot mutate prior data."""
    validate_snapshot(snapshot)
    target.parent.mkdir(parents=True, exist_ok=True)
    payload = json.dumps(snapshot, ensure_ascii=False, indent=2) + "\n"
    fd, tmp_name = tempfile.mkstemp(prefix=target.name + ".", dir=target.parent, text=True)
    try:
...
def collect_events(events: Iterable[dict[str, Any]], diary_date: date, closed_at: datetime | None = None) -> dict[str, Any]:
...
def _github_items(repo: str, token: str, start: datetime) -> list[dict[str, Any]]:
    params = urlencode({"state": "all", "since": start.isoformat(), "per_page": 100})
    req = Request(f"https://api.github.com/repos/{repo}/issues?{params}", headers={"Authorization": f"Bearer {token}", "Accept": "application/vnd.github+json", "User-Agent": "sado-development-diary"})
...
def main() -> int:
    ...
    parser.add_argument("--events-json", help="deterministic normalized event input; production adapters may generate this")
    ...
    if args.events_json:
        ...
        if not args.repo or not token:
            parser.error("--events-json or both --repo/GITHUB_REPOSITORY and GITHUB_TOKEN are required")
        raw = _github_items(args.repo, token, calc_window(diary_date)[0])
        events = []
        for item in raw:
            if item.get("pull_request") and item.get("created_at"):
                events.append({"kind": "PR_OPEN", "ref": f"pr:{item['number']}", "occurred_at": item["created_at"], "executor": (item.get("user") or {}).get("login") or "UNKNOWN", "task_class": "UNKNOWN"})

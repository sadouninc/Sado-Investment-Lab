...
def _candidate(raw: Mapping[str, Any]) -> RouterCandidate:
    return RouterCandidate(
        ...
        preflight_valid=raw.get("preflight_valid") is True,
        dependencies_satisfied=raw.get("dependencies_satisfied") is True,
        owner_conflict=raw.get("owner_conflict") is True,
        path_conflict=raw.get("path_conflict") is True,
        authority=str(raw.get("authority", "UNKNOWN")).strip().upper(),
        broadcast_sync_verified=raw.get("broadcast_sync_verified") is True,
    )
...
def select_route(candidates: Iterable[Mapping[str, Any]], *, provider_health: Mapping[str, Mapping[str, Any]], executor_order: Iterable[str] = DEFAULT_EXECUTOR_ORDER) -> dict[str, Any]:
    ...
    for raw in candidates:
        ...
        if not item.work_ref or not item.base_sha or not item.preflight_valid:
            blocked.append("PREFLIGHT_INVALID")
            continue
        if not item.broadcast_sync_verified:
            blocked.append("BROADCAST_SYNC_UNVERIFIED")
            continue
        if item.authority not in ALLOWED_AUTHORITIES:
            blocked.append("OWNER_AUTHORITY" if item.authority == "OWNER_AUTHORITY" else "AUTHORITY_UNKNOWN")
            continue
...
def issue_lease(selection: Mapping[str, Any], *, assigned_at: datetime) -> dict[str, Any]:
    ...
    required = ("work_ref", "executor", "task_class", "allowed_paths", "forbidden_paths", "base_sha", "fallback_order")
    ...
    lease_id = "lease-" + sha256(material.encode("utf-8")).hexdigest()[:32]
    return {"lease_id": lease_id, "work_ref": work_ref, "executor": executor, "task_class": str(selection["task_class"]), "assigned_at": assigned.isoformat(), "ack_deadline": ack_deadline.isoformat(), "execution_evidence_deadline": None, "allowed_paths": tuple(selection["allowed_paths"]), "forbidden_paths": tuple(selection["forbidden_paths"]), "base_sha": base_sha, "fallback_order": tuple(selection["fallback_order"]), "terminal_state": None}

...
def test_sony_exact_identity_fresh_close_is_usable() -> None:
    record = canonical_from_daily_record(_sony_daily(), security_id="JP:6758")
...
def test_symbol_or_security_identity_mismatch_is_blocked() -> None:
    record = canonical_from_daily_record(_sony_daily(), security_id="JP:6758")
...
def test_stale_source_is_blocked() -> None:
    record = canonical_from_daily_record(
        _sony_daily(source_timestamp="2026-08-16T13:00:00+09:00"),
        security_id="JP:6758",
    )
    result = evaluate_price_identity_gate(record, expected=_expected())
...
def test_split_basis_mismatch_is_blocked() -> None:
    record = canonical_from_daily_record(_sony_daily(basis="raw"), security_id="JP:6758")
    result = evaluate_price_identity_gate(record, expected=_expected())
...
def test_naive_source_timestamp_is_unknown_and_blocked() -> None:
    record = canonical_from_daily_record(
        _sony_daily(source_timestamp="2026-08-15T08:00:00"),
        security_id="JP:6758",
    )
    result = evaluate_price_identity_gate(record, expected=_expected())
...
def test_provider_ok_is_explicitly_preserved() -> None:
    record = canonical_from_daily_record(_sony_daily(), security_id="JP:6758", provider_status=PROVIDER_OK)
    result = evaluate_price_identity_gate(record, expected=_expected())
